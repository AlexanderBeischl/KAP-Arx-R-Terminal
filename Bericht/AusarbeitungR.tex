...
\noindent
In der heutigen medizinischen Forschung ist die statistische Auswertung von Patientendaten ein wichtiger Bestandteil. Hierbei müssen die verwendeten Daten zur Wahrung der Identität und des Datenschutzes der Patienten anonymisiert werden.

Das "`Data Anonymization Tool"' (\textit{ARX}), welches am Institut für Medizinische Statistik und Epidemiologie entwickelt wurde, ermöglicht diese Anonymisierung der Daten. Allerdings kommt es bei jeder Anonymisierung zu einem Informationsverlust und Verzerrungen der statistischen Eigenschaften. 
Um diese zu untersuchen und die Unterschiede der Eingabedaten sowie der anonymisierten Ausgabedaten zu analysieren, bietet sich die Nutzung der Programmiersprache und Entwicklungsumgebung \textit{R} an, da sie umfangreiche statistische Funktionen bereitstellt.
Allerdings ist eine gemeinsame Nutzung von \textit{R} in Verbindung mit \textit{ARX} aufgrund von Lizenzrechten nicht möglich. Ebenso können bestehende Schnittstellenprogramme aus Lizenzrechten nicht genutzt werden.

Aus diesem Grund wurde im Rahmen des Klinisches Anwendungsprojektes die \textit{Java}-Software \textit{R-Terminal} implementiert. Dieses Programm integriert \textit{R} in \textit{Java}, sodass \textit{R}-Befehle und \textit{R}-Skripte in \textit{Java} ausgeführt werden können und bietet somit eine lizenzfreie Plattform an. Hierfür wird durch das \textit{R-Terminal} eine Standalone-Installation von \textit{R} durch \textit{ProcessBuilder} als Prozess gestartet und die Kommunikation mittels Input- und Output-Stream hergestellt.
...
Mit dem \textit{R-Terminal} können außerdem auf Tabellen, welche aus dem  Anonymisierungs-Tool \textit{ARX} exportiert wurden, \textit{R}-Befehle ausgeführt werden. 

Somit können die Ein- und Ausgabedaten von \textit{ARX} mit \textit{R} überprüft und analysiert werden, ohne dass Lizenzkollisionen auftreten.
...
Hierfür wurde das "`Data Anonymization Tool"' (\textit{ARX}) vom Institut für Medizinische Statistik und Epidemiologie der Technischen Universität München entwickelt. Es ermöglicht die Anonymisierung der Patientendaten sowie mit diesen als Tabellen zu arbeiten.

Diese Notwendigkeit zur Wahrung der Privatsphäre der Patienten führt allerdings auch zu Informationsverlust und einer Verzerrung statistischer Eigenschaften. Die Anonymisierung eines Datensatzes kann somit dazu führen, dass dieser unter Umständen nicht mehr nutzbar ist. Deshalb müssen außerdem untersucht werden können, ob die Daten weiterhin für den geplanten Forschungszweck verwendbar sind.

 Obwohl \textit{ARX} bereits grundlegenden statistischen Methoden zur Analyse bereitstellt, bietet es sich an die Programmiersprache und Entwicklungsumgebung \textit{R} zu verwenden, um mit den Patientendaten aus \textit{ARX} in Form von Tabellen zu arbeiten und statistische Analysen durchzuführen.
 \textit{R} bietet als spezialisierte Statistiksoftware eine umfangreiche Sammlung an Funktionen zur statistischen Auswertung von Daten an und ist das wichtigste Werkzeug für Statistikanalysen der medizinischen Forschung. Deshalb 

Zusätzlich kann das \textit{R}-Pakets \textit{sdcMicro} verwendet werden, welches sowohl Funktionen zur Anonymisierung, als auch Methoden zur Risikoeinschätzung von Anonymisierung breitstellt \cite{sdc}. Dadurch können Datensätze direkt in \textit{R} anonymisiert und statistisch untersucht werden, wodurch potenzielle Abweichungen ausfindig gemacht werden können. 

Durch die Verwendung von \textit{R} sollen somit die Eingabe- und anonymisierte Ausgabe der Patientendaten aus \textit{ARX}
 untersucht werden, um die statistischen Abweichungen zu analysieren und die Verwendbarkeit der anonymisierten Daten zu untersuchen.
 
\subsection*{Möglichkeiten der Integration}

Eine direkte Analyse der Tabellen aus \textit{ARX} mit \textit{R} ist aufgrund der jeweiligen Lizenzierung leider nicht möglich, was nicht technische, sondern rechtliche Gründe hat.
...
Deshalb kann die von \textit{R} angebotene Benutzeroberfläche nicht gemeinsam mit \textit{ARX} genutzt werden.

Das Ziel des in diesem Rahmen entwickelten \textit{RTerminals} ist es, eine lizenzfreie Plattform zur Verfügung zu stellen, welche es ermöglicht \textit{R} aufzurufen und innerhalb des \textit{RTerminals} \textit{ARX}-Daten einzulesen und diese zu bearbeiten.
...
Es existieren zwar bereits für \textit{Java} Pakete wie \textit{RServe}, \textit{JRI} und \textit{RCaller}, welche ebenfalls eine Integration realisieren, eine direkten Nutzung von JRI, RServe und RCaller mit \textit{ARX} ist aufgrund der jeweiligen Lizenzierung aber nicht möglich.
