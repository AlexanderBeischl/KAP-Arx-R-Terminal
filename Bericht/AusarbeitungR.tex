...
Aus diesem Grund wurde im Rahmen des Klinisches Anwendungsprojektes die \textit{Java}-Software \textit{R-Terminal} implementiert, welches im Rahmen dieser Ausarbeitung vorgestellt wird. Dieses Programm integriert \textit{R} in \textit{Java}, sodass \textit{R}-Befehle und \textit{R}-Skripte in \textit{Java} ausgeführt werden können und bietet somit eine lizenzfreie Plattform an. Hierfür wird durch das \textit{R-Terminal} eine Standalone-Installation von \textit{R} durch \textit{ProcessBuilder} als Prozess gestartet und die Kommunikation mittels Input- und Output-Stream hergestellt.
...
Deshalb kann die von \textit{R} angebotene Benutzeroberfläche nicht genutzt werden, um \textit{ARX}-Datensätze auszuwerten.
